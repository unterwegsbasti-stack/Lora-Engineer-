...
import { Cpu, Layers, Sparkles, HardDrive, Download, FileText, Wand2, Smartphone, ShieldCheck, BookOpen } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <nav className="flex items-center gap-1 bg-slate-950/80 p-1 rounded-xl border border-slate-800/80 w-full md:w-auto overflow-x-auto">
          ...
          {onOpenDocs && (
            <button
              onClick={onOpenDocs}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-indigo-300 hover:text-white bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 transition-all whitespace-nowrap"
              title="Lizenz & Dokumentation anzeigen"
            >
              <ShieldCheck className="w-4 h-4 text-emerald-400" />
              <span>Lizenz & Docs</span>
            </button>
          )}
        </nav>
      </div>
